const { PricingPolicy, GoldWeb, WebInv, WebChangeLog } = require('../models');
...
module.exports = {
    ...
    `;
			...
			const variant = variantResp.data.data.productVariants.edges[0]?.node;
			if (!variant)
				return res
					.status(404)
			...
			if (tagPriceRaw) {
				try {
					...
					return res.status(400).json({ error: 'Invalid tag_price format' });
				}
			}

			const autoUpdate = getVariantValue('variant', 'autoUpdatePrice');
			...
			if (matchedPolicy.Type === 'PerGram') {
				...
				calculatedPrice = Math.round(
					tag_price - (tag_price * matchedPolicy.DiscountOnMargin) / 100
				);
			}
    ...
    `;
			...
			const errors = updateResp.data.data.productVariantUpdate.userErrors;
			if (errors.length) {
	...
	async syncWebInvFromShopify(req, res) {
		...
		try {
			console.log('Starting WebInv sync with fixed metafield queries...');
          ...
          productVariants(first: $limit, after: $cursor) {
            ...
            edges {
              node {
				...
				if (remainingPoints) {
					const [used, total] = remainingPoints.split('/').map(Number);
					console.log(`API usage: ${used}/${total}, Request #${requestCount}`);
				...
				for (const edge of variants) {
					...
					const getValue = (metafieldObj) => metafieldObj?.value || null;
					...
					try {
						...
						const autoUpdateRaw = getValue(node.product.autoUpdatePrice);
						if (autoUpdateRaw !== null) {
							if (typeof autoUpdateRaw === 'string') {
								autoUpdatePrice = autoUpdateRaw.toLowerCase() === 'true';
	...
	async updatePricesByClasscode(req, res) {
		try {
			...
			if (fromClasscode > toClasscode) {
				return res.status(400).json({
					error: 'fromClasscode must be less than or equal to toClasscode',
				});
			}
			...
			const eligibleProducts = await WebInv.find({
				...
				grossWeight: { $gt: 0 },
				$or: [{ tag_price: { $gt: 0 } }, { gold_karat: { $exists: true } }],
			}).lean();

			if (eligibleProducts.length === 0) {
				return res.status(404).json({
					error: 'No eligible products found in the specified classcode range',
				});
			}
			...
			for (const product of eligibleProducts) {
				try {
					...
					if (!matchedPolicy) {
						...
						try {
							await WebChangeLog.logChanges(userId, 'shopify', product.sku, {
								fieldName: 'price',
					...
					if (matchedPolicy.Type === 'PerGram') {
						...
						calculatedPrice = Math.round(
							product.tag_price -
								(product.tag_price * matchedPolicy.DiscountOnMargin) / 100
						);
					}
					...
					if (calculatedPrice > 0) {
						...
						priceUpdates.push({
							...
							priceChange: calculatedPrice - (product.currentPrice || 0),
							classcode: product.classcode,
						...
						try {
							await WebChangeLog.logChanges(userId, 'shopify', product.sku, {
								...
								newValue: 0,
								source: 'bulk_pricing',
								message: 'Price Update Failed: Calculated Price Invalid',
							});
						} catch (logError) {
							console.error(
			...
			if (dryRun) {
				...
				const updatesByClasscode = priceUpdates.reduce((acc, update) => {
				...
				const avgPriceChange =
					priceChanges.length > 0
						...
						: 0;
				const totalPriceChange = priceChanges.reduce((a, b) => a + b, 0);
			...
			console.log(`Starting Shopify updates in batches of ${batchSize}...`);
        ...
        `;
				...
				try {
					...
					if (successfulUpdates.length > 0) {
						console.log(
							...
							} successful changes for batch ${Math.floor(i / batchSize) + 1}`
						);
					}
			...
			const successfulByClasscode = priceUpdates
				.filter(
					(update) => !shopifyErrors.find((err) => err.sku === update.sku)
				)
				.reduce((acc, update) => {
					acc[update.classcode] = (acc[update.classcode] || 0) + 1;
					return acc;
	...
	async getSkuDetails(req, res) {
		try {
			...
			if (!productId || !variantId) {
				return res.status(400).json({
					error: 'SKU missing productId or variantId - sync may be required',
				});
			}
				...
				data: {
					...
					product: {
						...
						productCategory: {
							...
							fullName: product.productCategory.productTaxonomyNode.fullName,
						},
					...
					recentChanges: recentChanges.map((change) => ({
						...
						user:
							change.user?.employeeId || change.user?.firstName || 'Unknown',
					...
					dataComparison: {
						priceMatch: localSkuData.currentPrice === parseFloat(variant.price),
	...
	async updateSkuDetails(req, res) {
		try {
			...
			Object.entries(changes.product.fields).forEach(([field, change]) => {
				changeLog.push({
					fieldName: field,
			...
			Object.entries(changes.variant.fields).forEach(([field, change]) => {
				changeLog.push({
					fieldName: field,
					oldValue: change.oldValue,
					newValue: change.newValue,
			...
			if (autoUpdatePrice) {
				const entryDate = currentData.product.metafields.sku?.entryDate;

				const classCode = currentData.product.metafields.sku?.classcode;

				const vendor = currentData.product.metafields.sku?.vendor;

				const grossWeight = currentData.product.metafields.sku?.grossWeight;

				const tagPrice = currentData.product.metafields.sku?.tag_price;

				const goldKarat = currentData.product.metafields.sku?.gold_karat;
				...
				if (matchedPolicy) {
					...
					if (matchedPolicy.Type === 'PerGram' && grossWeight) {
						...
						calculatedPrice = Math.floor(
							tagPrice - (tagPrice * matchedPolicy.DiscountOnMargin) / 100
						);
					}
					...
					if (calculatedPrice !== currentData.variant.price) {
						...
						if (matchedPolicy.Type === 'PerGram') {
							changeLog.push({
								...
								oldValue: Math.floor(currentData.variant.price / grossWeight),
							...
							changeLog.push({
								...
								newValue: Math.floor((calculatedPrice / tagPrice) * 100),
								source: 'Auto Pricing',
								message: 'Price auto-calculated based on policy',
							});
						}
		...
		try {
          ...
          `,
			...
			for (const variant of top30) {
				...
				await log(`✅ Tagged: ${variant.id} — ${variant.upload_date}`);
			}

			await log('🟡 Bulk job started. Waiting for completion in background.');
	...
	async priceRefresh(req, res) {
		...
		try {
              ...
              edges {
				...
				onEachRecord: async (variant) => {
					...
					if (productId) {
						...
						productOptions = productMeta.options;
					}
					...
					const variantData = {
						...
						autoUpdate: (() => {
							const value = variant.autoUpdatePrice?.value;
							if (!value) return false;
							// Parse string to actual boolean
			...
			// 🔍 DEBUG: Check first 5 variants and find one with autoUpdate enabled
			const autoUpdateEnabled = variantBuffer.filter((v) => v.autoUpdate);
			await log(
				`📊 Variants with autoUpdate enabled: ${autoUpdateEnabled.length}`
			);

			if (autoUpdateEnabled.length > 0) {
				const sample = autoUpdateEnabled[0];
				await log(`🔍 Sample variant with autoUpdate=true:`);
				await log(`   SKU: ${sample.sku}`);
				await log(`   classcode: ${sample.classcode}`);
				await log(`   goldKt: ${sample.goldKt}`);
				await log(`   grossWeight: ${sample.grossWeight}`);
				await log(`   entryDate: ${sample.entryDate}`);
				await log(`   vendor: ${sample.vendor}`);
				await log(`   autoUpdate: ${sample.autoUpdate}`);
				await log(`   current_price: ${sample.current_price}`);
				await log(`   tag_price: ${sample.tag_price}`);
			} else {
				await log(
					`⚠️ NO VARIANTS have autoUpdate enabled! Check your metafield values.`
				);
			}
			...
			const result = await batchUpdateVariantsGraphQL(variantsToUpdate, log);
			...
			// ✅ Response already sent above - job runs in background
		} catch (error) {
...
async function enrichWithMetafields(tempArr, productId) {
	...
	const getProductValue = (namespace, key) =>
		productMetafields.find((m) => m.namespace === namespace && m.key === key)
			?.value || null;
		...
		tempArr.map(async (variant) => {
			try {
				...
				if (!variant.autoUpdate) {
					await log(`⚠️ Skipping ${variant.sku}: AutoUpdate disabled`);
					return {
						...variant,
						needsUpdate: false,
				...
				if (!variant.goldKt) {
					await log(`⚠️ Skipping ${variant.sku}: Missing gold karat`);
					return {
						...variant,
						needsUpdate: false,
				...
				if (!variant.grossWeight || isNaN(variant.grossWeight)) {
					await log(`⚠️ Skipping ${variant.sku}: Missing/invalid gross weight`);
					return {
						...variant,
						needsUpdate: false,
				...
				if (!variant.entryDate) {
					await log(`⚠️ Skipping ${variant.sku}: Entry date is missing`);
					return {
						...variant,
						needsUpdate: false,
				...
				await log(
					`🔍 Looking for policy: SKU=${variant.sku}, Classcode=${
						variant.classcode
					}, Vendor=${variant.vendor}, Age=${productAgeMonths.toFixed(
						1
					)} months`
				);
				...
				await log(
					`📋 Found ${policies.length} policies for classcode ${variant.classcode}`
				);
				...
				if (!matchedPolicy) {
					await log(
						`❌ Skipping ${
							variant.sku
						}: No matching pricing policy found (Classcode: ${
							variant.classcode
						}, Vendor: ${variant.vendor}, Age: ${productAgeMonths.toFixed(
							1
						)}mo)`
				...
				await log(
					`✅ Matched policy: ${matchedPolicy.Name} (Type: ${matchedPolicy.Type})`
				);
				...
				if (matchedPolicy.Type === 'PerGram') {
					if (variant.goldKt === '22KT') {
						...
						await log(
							`⚠️ Skipping ${variant.sku}: Unsupported gold karat ${variant.goldKt}`
						);
						return {
							...variant,
							needsUpdate: false,
					...
					if (!variant.tag_price) {
						await log(
							`⚠️ Skipping ${variant.sku}: Tag price required for discount calculation but missing`
						);
						return {
							...variant,
							needsUpdate: false,
					...
					calculatedPrice = Math.round(
						variant.tag_price -
							(variant.tag_price * matchedPolicy.DiscountOnMargin) / 100
					);
					perGramOrDisc = matchedPolicy.DiscountOnMargin;
				} else {
					await log(
						`⚠️ Skipping ${variant.sku}: Unknown policy type ${matchedPolicy.Type}`
					);
					return {
						...variant,
						needsUpdate: false,
				...
				const priceChanged =
					Math.floor(currentPrice) !== Math.floor(calculatedPrice);

				await log(
					`💰 ${variant.sku}: Current=$${currentPrice}, Calculated=$${calculatedPrice}, Changed=${priceChanged}`
				);
				...
				await log(
					`❌ Error calculating price for variant ${variant.sku}: ${error.message}`
				);
				console.error(error);
				return {
					...variant,
...
async function updateVariantPrices(tempArr, log = console.log) {
	const variantsToUpdate = tempArr.filter((variant) => variant.needsUpdate);
	...
	const updatePromises = variantsToUpdate.map(async (variant) => {
		try {
			...
			await log(`✅ Updated ${variant.sku} → $${variant.calculated_price}`);
			...
			await log(`✅ Updated ${variant.sku} → $${variant.perGramOrDisc}`);
		} catch (error) {
			console.error(`Error updating variant ${variant.sku}:`);
			await log(`❌ Failed to update ${variant.sku}: ${error.message}`);
